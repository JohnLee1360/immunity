...
class BaseAggregatorClient(Aggregator):
    ...
    User API
    """
    def set_attack_weight(self, model):
        target_weight = copy.deepcopy(model.state_dict())
        for key in target_weight.keys():
            layer_num = target_weight[key].size()
            target_weight[key] = torch.normal(0, 0.1, layer_num)

        return target_weight

    def model_aggregation(self, ctx, model, state):
        # 修改一 记录模型第一层参数
        ...
        # 设置client1 为攻击者
        if int(u) == 1:
            myweight = copy.deepcopy(model.state_dict())
            if cur_epoch == 1:
                self.attack_weight = self.set_attack_weight(model)
            for key in myweight.keys():
                layer_num = myweight[key].size()
                noise = (0.95)**cur_epoch*(torch.normal(0, 0.1, layer_num))
                # myweight[key] = self.attack_weight[key] + noise
                gt = (0.9)**cur_epoch
                myweight[key] = gt*myweight[key] + (1 - gt)*(self.attack_weight[key] + noise)
            for i,k in enumerate(model_keys):
                to_send[i] = myweight[k].detach().double().numpy()
        weight_layer0s = to_send[0]
        cur_dir = os.getcwd()
        model_dir = os.path.join(cur_dir, 'result', 'to_send', u)
        ...
        agg_model = self.model_aggregator.secure_aggregate(model, ctx, to_send, self._weight, cur_epoch)
    ...
    def loss_aggregation(self, ctx, loss):
        if isinstance(loss, t.Tensor):
            ...
            loss = np.array(loss)
        loss = [loss]
        agg_loss = self.loss_aggregator.loss_aggregate(ctx, loss, self._weight)
        return agg_loss
...
class BaseAggregatorServer(Aggregator):
    ...
    def loss_aggregation(self, ctx, ranks=None):
        return float(self.loss_aggregator.loss_aggregate(ctx, ranks=ranks)[0])
